...
void FCriWarePluginSettingsDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (!CriWareSettings->IsMixedConfigFiles()){
		...
		IDetailCategoryBuilder& ManaPluginCategory = DetailBuilder.EditCategory(TEXT("Mana"));
#if (CRIWARE_USE_ADX_LIPSYNC)
		IDetailCategoryBuilder& AdxLipSyncExpansionCategory = DetailBuilder.EditCategory(TEXT("ADX LipSync"));
#endif	/* <CRIWARE_USE_ADX_LIPSYNC> */
...
#if !defined(CRIWARE_UE4_LE)	/* <cri_delete_if_LE> */
	// Customization for startup movies. 
	// As Unreal startup movies, a widget force to copy movies to Content/Movies folder.
	IDetailCategoryBuilder& ManaPluginCategory = DetailBuilder.EditCategory(TEXT("Mana"));
	// Set properties in correct order.
	ManaPluginCategory.AddProperty("bUseManaStartupMovies", (UClass*)0, NAME_None, EPropertyLocation::Advanced);
	ManaPluginCategory.AddProperty("bWaitForMoviesToComplete", (UClass*)0, NAME_None, EPropertyLocation::Advanced);
	ManaPluginCategory.AddProperty("bMoviesAreSkippable", (UClass*)0, NAME_None, EPropertyLocation::Advanced);
	// Add movies array with widget
	StartupMoviesPropertyHandle = DetailBuilder.GetProperty("StartupMovies");
	...
	StartupMoviesBuilder->OnGenerateArrayElementWidget(FOnGenerateArrayElementWidget::CreateSP(this, &FCriWarePluginSettingsDetails::GenerateArrayElementWidget));
	// Add this CustomBuilder to Mana Category "Advanced".
	const bool bForAdvanced = true;
	ManaPluginCategory.AddCustomBuilder(StartupMoviesBuilder, bForAdvanced);
#endif	/* </cri_delete_if_LE> */
}
...
void FCriWarePluginSettingsDetails::GenerateArrayElementWidget(TSharedRef<IPropertyHandle> PropertyHandle, int32 ArrayIndex, IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	IDetailPropertyRow& FilePathRow = ChildrenBuilder.AddProperty(PropertyHandle);
	{
		...
		.ValueContent()
			...
			.FilePath(this, &FCriWarePluginSettingsDetails::HandleFilePathPickerFilePath, PropertyHandle)
			.FileTypeFilter(TEXT("Sofdec Movie (*.usm)|*.usm"))
			.OnPathPicked(this, &FCriWarePluginSettingsDetails::HandleFilePathPickerPathPicked, PropertyHandle)
			];
	}
#endif	/* </cri_delete_if_LE> */
};
