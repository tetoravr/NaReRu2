...
#include <cri_mana_ps5.h>
#include <cri_file_system_ps5.h>
...
#elif defined(XPT_TGT_SCARLETT)
#include <cri_atom_scarlett.h>
#include <cri_mana_scarlett.h>
...
#if defined(XPT_TGT_PS4) || defined(XPT_TGT_PS5) \
	|| defined(XPT_TGT_XBOXONE) || defined(XPT_TGT_SCARLETT)
#define CRIWARE_UE4_MAX_CHANNELS 8
#elif defined(XPT_TGT_SWITCH)
...
#elif defined(XPT_TGT_PS5)
#define CriAtomExConfig_UE4				CriAtomExConfig_PS5
#define criAtomEx_SetDefaultConfig_UE4	criAtomEx_SetDefaultConfig_PS5
#define criAtomEx_Initialize_UE4		criAtomEx_Initialize_PS5
#define criAtomEx_Finalize_UE4			criAtomEx_Finalize_PS5
#elif defined(XPT_TGT_XBOXONE)
...
#elif defined(XPT_TGT_SCARLETT)
#define CriAtomExConfig_UE4				CriAtomExConfig_SCARLETT
#define criAtomEx_SetDefaultConfig_UE4	criAtomEx_SetDefaultConfig_SCARLETT
#define criAtomEx_Initialize_UE4		criAtomEx_Initialize_SCARLETT
#define criAtomEx_Finalize_UE4			criAtomEx_Finalize_SCARLETT
#elif defined(XPT_TGT_SWITCH)
...
#elif defined(XPT_TGT_PS5)
	/* スレッドプライオリティの設定 */
	criAtomEx_SetServerThreadPriority_PS5(AtomServerThreadPriority_PS4);
	criAtomEx_SetOutputThreadPriority_PS5(AtomOutputThreadPriority_PS4);
	criFs_SetFileAccessThreadPriority_PS5(FsFileAccessThreadPriority_PS4);
	criFs_SetDataDecompressionThreadPriority_PS5(FsDataDecompressionThreadPriority_PS4);
	criFs_SetMemoryFileSystemThreadPriority_PS5(FsMemoryFileSystemThreadPriority_PS4);

	/* コアアフィニティの設定 */
	criAtomEx_SetServerThreadAffinityMask_PS5(AtomServerThreadAffinityMask_PS4);
	criAtomEx_SetOutputThreadAffinityMask_PS5(AtomOutputThreadAffinityMask_PS4);
	criFs_SetFileAccessThreadAffinityMask_PS5(FsFileAccessThreadAffinityMask_PS4);
	criFs_SetDataDecompressionThreadAffinityMask_PS5(FsDataDecompressionThreadAffinityMask_PS4);
	criFs_SetMemoryFileSystemThreadAffinityMask_PS5(FsMemoryFileSystemThreadAffinityMask_PS4);
#endif
#endif	/* </cri_delete_if_LE> */
	...
	if (bInitializeMana) {
		...
		if (bManaUseH264Decoder) {
		...
		#else
			{
				...
				CriManaMediaFoundationH264DecoderConfig_PC config = {};
				criMana_SetDefaultMediaFoundationH264DecoderConfig_PC(&config);
				criMana_SetupMediaFoundationH264Decoder_PC(&config, NULL, 0);
			}
		#endif
	#endif
#elif defined(XPT_TGT_XBOXONE)
			...
			{
				CriManaH264DecoderConfig_XBOXONE config = {};
				criMana_SetDefaultH264DecoderConfig_XBOXONE(&config);
				criMana_SetupH264Decoder_XBOXONE(&config, NULL, 0);
			}
	#endif
#endif
		}
...
#if CRIWARE_USE_MANA_VP9
		{
			...
			vp9_config.mem_alloc_func = static_cast<CriManaMallocFunc>(FCriWareAllocator_Native::AllocateMemory);
			vp9_config.mem_free_func = static_cast<CriManaFreeFunc>(FCriWareAllocator_Native::FreeMemory);

			auto PlatformMana = UCriWareInitializer::Platform.Mana();
			if (PlatformMana) {
				vp9_config.mem_usr_obj = &PlatformMana->GetMemoryAllocator(ECriWareMemoryType::ManaVp9Memory);
			} else {
				vp9_config.mem_usr_obj = &UCriWareInitializer::DefaultAllocator;
			}
