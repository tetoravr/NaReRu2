...
FExternalOESTextureRenderer::FExternalOESTextureRenderer(bool bUseOwnContext, bool bShouldUseVulkan, bool bShouldUseDesktopOpenGL)
: TextureID(-1)
, FBO(-1)
, ReadTexture(0)
...
, bUseVulkan(bShouldUseVulkan)
, bUseDesktopOpenGL(bShouldUseDesktopOpenGL)
, bInitialized(false)
...
, vb1pointer(nullptr)
{
	...
	BlitFragmentShaderBGRA = 
		...
		FString(TEXT("void main()\n")) +
		FString(TEXT("{\n"));
		if (bUseVulkan || bUseDesktopOpenGL)
		{
...
bool FExternalOESTextureRenderer::InitContext()
{
	if (Context == EGL_NO_CONTEXT && bUseIsolatedContext)
	{
		...
		EGLConfig* EGLConfigList = new EGLConfig[eglNumConfigs];
...
void FExternalOESTextureRenderer::MakeCurrent()
{
	if (bUseIsolatedContext)
	{
		...
		{
			UE_LOG(LogCriWareAndroid, Error, TEXT("Error setting media player context."));
		}
	}
}
...
void FExternalOESTextureRenderer::RestoreContext()
{
	if (bUseIsolatedContext)
	{
		...
		if (bResult == EGL_FALSE)
		{
			UE_LOG(LogCriWareAndroid, Error, TEXT("Error setting unreal context."));
		}
	}
}
...
void FExternalOESTextureRenderer::InitSurfaceTexture()
{
	...
	{
		UE_LOG(LogCriWareAndroid, Error, TEXT("Could not link program"));
		...
		UE_LOG(LogCriWareAndroid, Error, TEXT("%s"), ANSI_TO_TCHAR(errorLog));
...
GLuint FExternalOESTextureRenderer::CreateShader(GLenum ShaderType, const FString& ShaderSource)
{
	...
	if (shader != 0)
	{
		...
		{
			UE_LOG(LogCriWareAndroid, Error, TEXT("Could not compile shader %d"), static_cast<int32>(ShaderType));
			int32 maxLength = 0;
...
bool FExternalOESTextureRenderer::CopyFrameTexture(int32 DestTexture, void *NativeBuffer, const FIntPoint& TextureDimensions, void* DestBuffer)
{
	if (!bInitialized)
	{
	...
	{
		UE_LOG(LogCriWareAndroid, Error, TEXT("gl error %d"), static_cast<int32>(error));
	}
	...
	if (image == EGL_NO_IMAGE_KHR)
	{
		...
		UE_LOG(LogCriWareAndroid, Error, TEXT("Failed to create EGLImage from the buffer. %s %d"), *ErrorText, errorcode);
		RestoreContext();
		return false;
	}
	glActiveTexture(GL_TEXTURE0);
...
// #if	UE_BUILD_DEBUG
	...
	{
		UE_LOG(LogCriWareAndroid, Warning, TEXT("Failed to complete framebuffer attachment (%d)"), static_cast<int32>(status));
	}
// #endif
